...
<body style="margin: 0; padding: 0;">
	<div id='page1' style='margin: 0 auto;'>
		...
		<c:forEach items="${userparts}" var="userpart" varStatus="status">
			...
			<div
				style="font-size: 10px; text-align: right; margin-top: 6px; width: 60px; float: left;">
				<c:if test="${flag1 != null && flag1 != '1003'}">${total }</c:if><!-- 这里有错,显示去掉抹0的钱-->
		...
		<div
			style="font-size: 10px; text-align: center; margin-top: 6px; width: 75px; float: left;">${total }</div><!--实缴金额  现在显示的未抹零的钱-->
		<div
		...
		<div
			style="font-size: 10px; text-align: center; margin-top: 6px; width: 80px; float: left;">
			<c:if test="${flag1 != null && flag1 != '1003'}">${inincard.money4 + total -nowMoney }</c:if>
		...
		<div
				style="font-size: 10px; text-align: center; margin-top: 6px; width: 80px; float: left;">是否免单:${miandan==true?"是":"否" }</div>
		<c:if
