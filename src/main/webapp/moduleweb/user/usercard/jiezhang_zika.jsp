...
//								}
							...
							function doJiezhangPasswd() {
								...
								ids = $("#myTable3").jqGrid('getDataIDs');
								for (var i = 0; i < ids.length; i++) {
								...
								$
										.post(
												"${ctxPath}/user/userIncard/jiezhangWithInincard.do",
												obj,
												function(data, status) {
													if (data.success) {
														layer.closeAll();
							...
							$("#incard #btnJiezhang")
									.click(
											function() { // 结账
												...
												$("#jiezhangDiv #money3")
														.val(0);
												$("#jiezhangDiv #incardId")
														.val("");

												// 子卡结账
												$("#jiezhangDiv #inincardId")
														.val("");
												$(".xiaocishu_zi").hide();
												$("#jiezhangDiv #xiaocishu_zi")
														.val(0);
												if (selectedInincardRowId) {
													$("#jiezhangDiv #xiaocishu")
															.val(1);
													var kd = $(
															"#"
							...
							$("#incard #btnPrint")
									.click(
											function() { // 打印小票
												...
												if ((ds && ds.length > 0)
														|| (wms && wms.length > 0)) {
													$("#prt")[0].src = '${ctxPath }/user/userpart/print.do?type=10&ids='
															+ ds.join(',')
															+ '&wmids='
															+ wms.join(',');
												} else {
							...
							$("#userpart #btnPrint").click(function() { // 打印小票 - 子卡
								$("#incard #btnPrint").click();
							});
							...
							function tichengYeji2(staffId, somemoney, yeji,
								...
								for(var i=0;i<fillmaps.smallsorts.length;i++){
								    if(ss==fillmaps.smallsorts[i]._id){
								       chengben = fillmaps.smallsorts[i].price_chengben;
									}
								}
								if (s1) {
							...
							$("#jiezhangDiv2").hide();
							$("#userpart #btnJiezhang").click(function() { // 结账 - 子卡
								$("#incard #btnJiezhang").click();
							});
	...
	<div id="jiezhangDiv" class="wrapper wrapper-content"
		...
		<div class="row" style="margin-top: 20px">
			<form id="jiezhangForm" method="post" class="form-horizontal"
				...
				<input id="incardId" name="incardId" type="hidden"> <input
					id="inincardId" name="inincardId" type="hidden">
				...
				<div class="hr-line-dashed"></div>
				<div class="form-group xiaocishu_zi">
					<label class="col-sm-4 control-label">子卡次数</label>
					<div class="col-sm-7">
						<input id="xiaocishu_zi" name="xiaocishu_zi" type="text"
							class="form-control" disabled>
					</div>
				</div>
