...
<body style="margin: 0; padding: 0;">
	<div id='page1' style='margin: 0 auto;'>
		...
		<div
			style="font-size: 10px; text-align: center; margin-top: 6px; width: 80px; float: left;">卡号${incard == null?inincard.id_2:incard.id_2 }</div>
		<c:if test="${incard != null }">
			<c:forEach items="${userparts}" var="userpart" varStatus="status">
				<c:if test="${userpart.type == 1 }">
					<c:set var="flag1" value="" />
					<c:forEach items="${usersorts}" var="usersort">
						<c:if test="${userpart.membersort == usersort._id }">
							<div
								style="font-size: 10px; text-align: center; margin-top: 6px; width: 70px; float: left;">${usersort.name1}</div>
					...
					<c:forEach items="${smallsorts}" var="smallsort">
						<c:if test="${userpart.smallsort == smallsort._id }">
							<div
								style="font-size: 10px; text-align: right; margin-top: 6px; width: 90px; float: left;">${smallsort.name}</div>
						</c:if>
					</c:forEach>
					...
					<c:set var="shijiao" value="${shijiao +  oneMoney}" />
		...
		<c:forEach items="${waimais}" var="waimai" varStatus="status">
			...
			<c:set var="shijiao" value="${shijiao +  waimai.money2}" />
			<div
		...
		<c:if test="${inincard != null }">
			<div
				style="font-size: 2px; float: left; width: 100%; margin-top: 10px; margin-bottom: 10px; border-top: 1px solid #000000;"></div>
			<div style="font-size: 12px; text-align: center;">子卡消费</div>
			<!--子卡初始化-->
			<c:set var="cishu" value="0" />
			<c:set var="total" value="0" />
			<c:set var="moling" value="0" />
			<c:forEach items="${userparts}" var="userpart" varStatus="status">
				<c:if test="${userpart.type == 11 }">
					<c:set var="flag1" value="" />
					<c:forEach items="${usersorts}" var="usersort">
						<c:if test="${userpart.membersort == usersort._id }">
					...
					<c:set var="total" value="${total +  userpart.money_xiaofei-userpart.money5}" />
					<c:set var="moling" value="${moling +  userpart.money5}" />
					...
					<div
						style="font-size: 10px; text-align: right; margin-top: 6px; width: 60px; float: left;">
						<c:if test="${flag1 != null && flag1 != '1003'}">${total }</c:if>
						<c:if test="${flag1 != null && flag1 == '1003'}">${userpart.cishu }次</c:if>
					</div>
					<div
							style="font-size: 10px; text-align: left; margin-top: 6px; width: 80px; float: left;">是否免单:${miandan==true?"是":"否" }</div>
			...
			<div
				style="font-size: 10px; text-align: center; margin-top: 6px; width: 75px; float: left;">${total }</div>
			<div
				style="font-size: 10px; text-align: right; margin-top: 6px; width: 35px; float: left;">${moling }</div>
			<div
				style="font-size: 10px; text-align: center; margin-top: 6px; width: 80px; float: left;">划卡</div>
			<div
				style="font-size: 10px; text-align: right; margin-top: 6px; width: 80px; float: left;">找零</div>
			<div
				style="font-size: 10px; text-align: center; margin-top: 6px; width: 80px; float: left;">
				<c:if test="${flag1 != null && flag1 != '1003'}">${total }</c:if>
				<c:if test="${flag1 != null && flag1 == '1003'}">${cishu }次</c:if>
			...
			<div
				style="font-size: 10px; text-align: right; margin-top: 6px; width: 80px; float: left;">${zhaoling }</div>
			...
			<div
				style="font-size: 10px; text-align: center; margin-top: 6px; width: 80px; float: left;">
				<c:if test="${flag1 != null && flag1 != '1003'}">${inincard.money4 + total }</c:if>
				<c:if test="${flag1 != null && flag1 == '1003'}">${inincard.shengcishu + cishu }次</c:if>
			</div>
			<div
				style="font-size: 10px; text-align: right; margin-top: 6px; width: 80px; float: left;">
				<c:if test="${flag1 != null && flag1 != '1003'}">${inincard.money4 }</c:if>
				<c:if test="${flag1 != null && flag1 == '1003'}">${inincard.shengcishu }次</c:if>
			</div>
		</c:if>
		<c:if
