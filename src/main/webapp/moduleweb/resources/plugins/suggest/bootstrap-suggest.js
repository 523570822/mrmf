	...
	if (typeof define === "function" && define.amd) {
		...
		(function($) {
			...
			function setValue($input, keywords, options) {
				...
				var separator = options.separator || ',', inputValList /*
																		 * ,
																		 * inputIdList
																		 */;
				...
				if (options && options.multiWord) { // 多关键字支持，只设置 val
					...
					$input.attr('data-id', keywords.id).focus().val(
							keywords.key);
				}
			...
			function adjustDropMenuPos($input, $dropdownMenu, options) {
				...
				if (options.autoDropup) {
					setTimeout(
							function() {
								var $inputGroup = $dropdownMenu
										.parents('.input-group');

								if ( // 自动判断菜单向上展开
								($window.height() + $window.scrollTop() - $input
										.offset().top) < $dropdownMenu.height()
										&& // 假如向下会撑长页面
										$input.offset().top > ($dropdownMenu
												.height() + $window.scrollTop()) // 而且向上不会撑到顶部
								) {
				...
				if (isIe && !options.showBtn) {
					if (!$dropdownMenu.parents('.input-group').hasClass(
							'dropup')) {
			...
			function setBackground($input, options) {
				...
				if ((options.indexId === -1 && !options.idField)
						|| options.multiWord) {
					return $input;
				}

				inputbg = $input.css('background-color').replace(/ /g, '')
						.split(',', 3).join(',');
				// console.log(inputbg);
			...
			function adjustScroll($input, $dropdownMenu, options) {
				// 控制滑动条
				var $hover = $input.parent().find(
						'tbody tr.' + options.listHoverCSS), pos, maxHeight;

				if ($hover.length) {
					pos = ($hover.index() + 3) * $hover.height();
					maxHeight = Number($dropdownMenu.css('max-height').replace(
							'px', ''));

					if (pos > maxHeight
							|| $dropdownMenu.scrollTop() > maxHeight) {
						pos = pos - maxHeight;
					} else {
						pos = 0;
					}
			...
			/**
			 * 验证 $input 对象是否符合条件 1. 必须为 bootstrap 下拉式菜单 2. 必须未初始化过
			 */
			...
			/**
			 * 数据格式检测 检测 ajax 返回成功数据或 data 参数数据是否有效 data 格式：{"value": [{},
			 * {}...]}
			 */
			...
			function inEffectiveFields(field, options) {
				...
				return !(field === '__index' || effectiveFields.length
			...
			/**
			 * 下拉列表刷新 作为 fnGetData 的 callback 函数调用
			 */
			function refreshDropMenu($input, data, options) {
				var $dropdownMenu = $input.parent().find('ul.dropdown-menu'), len, i, j, index = 0, tds, html = [ '<table class="table table-condensed table-sm" style="margin:0">' ], idValue, keyValue; // 作为输入框
																																																			// data-id
																																																			// 和内容的字段值
				...
				// 相同数据，不用继续渲染了
				if (options._lastData
						&& JSON.stringify(options._lastData.value) === JSON
								.stringify(dataList)
				...
				if (options.showHeader) {
					...
					for (j in dataList[0]) {
						...
						html.push('<th>',
								(options.effectiveFieldsAlias[j] || j),
				...
				for (i = 0; i < len; i++) {
					...
					html.push('<tr data-index="', (dataI.__index || i),
							'" data-id="', idValue, '" data-key="', keyValue,
							'">', tds.join(''), '</tr>');
				}
				...
				setTimeout(
						function() {
							...
							if ($dropdownMenu.height() < $table.height()
									&& Number($dropdownMenu.css('min-width')
											.replace('px', '')) < $dropdownMenu
											.width()) {
								pdr = 18;
								mgb = 20;
							}
			...
			function ajax(options, keyword) {
				...
				if ($.isFunction(options.fnAdjustAjaxParam)) {
					ajaxParam = $.extend(ajaxParam, options.fnAdjustAjaxParam(
							keyword, options));
				}
			...
			function isInWord(keyword, key, value, options) {
				...
				return value
						&& (inEffectiveFields(key, options) || inSearchFields(
								key, options)) && // 必须在有效的搜索字段中
			...
			function getData(keyword, $input, callback, options) {
				...
				if (options.url) {
					ajax(options, keyword).done(function(result) {
						callback($input, options.data, options); // 为
																	// refreshDropMenu
			...
			/**
			 * 数据处理 url 获取数据时，对数据的处理，作为 fnGetData 之后的回调处理
			 */
			...
			function getIClear($input, options) {
				...
				if (options.clearable && !$iClear.length) {
					$iClear = $(
							'<i class="clearable glyphicon glyphicon-remove"></i>')
						...
						.css(
								{
									position : 'absolute',
									top : 12,
									right : options.showBtn ? ($input.next(
											'.input-group-btn').width() || 33) + 2
											: 12,
									zIndex : 4,
									cursor : 'pointer',
									fontSize : 12
			...
			var defaultOptions = {
				...
				indexId : 0, // 每组数据的第几个数据，作为input输入框的 data-id，设为 -1 且
								// idField 为空则不设置此值
				...
				getDataMethod : 'firstByUrl', // 获取数据的方式，url：一直从url请求；data：从
												// options.data
												// 获取；firstByUrl：第一次从Url获取全部数据，之后从options.data获取
				delayUntilKeyup : false, // 获取数据的方式 为 firstByUrl
											// 时，是否延迟到有输入时才请求数据
				ignorecase : false, // 前端搜索匹配时，是否忽略大小写
				effectiveFields : [], // 有效显示于列表中的字段，非有效字段都会过滤，默认全部有效。
				effectiveFieldsAlias : {}, // 有效字段的别名对象，用于 header 的显示
				searchFields : [], // 有效搜索字段，从前端搜索过滤数据时使用，但不一定显示在列表中。effectiveFields
									// 配置字段也会用于搜索过滤
				twoWayMatch : true, // 是否双向匹配搜索。为 true 即输入关键字包含或包含于匹配字段均认为匹配成功，为
									// false 则输入关键字包含于匹配字段认为匹配成功
				multiWord : false, // 以分隔符号分割的多关键字支持
				separator : ',', // 多关键字支持时的分隔符，默认为半角逗号

				/* UI */
				autoDropup : false, // 选择菜单是否自动判断向上展开。设为
									// true，则当下拉菜单高度超过窗体，且向上方向不会被窗体覆盖，则选择菜单向上弹出
				...
				fnAdjustAjaxParam : null, // 调整 ajax
											// 请求参数方法，用于更多的请求配置需求。如对请求关键字作进一步处理、修改超时时间等
			...
			var methods = {
				init : function(options) {
					...
					if (undefined === options.showHeader
					...
					return self
							.each(function() {
								var $input = $(this), $iClear = getIClear(
										$input, options), mouseenterDropdownMenu, keyupTimer, // keyup
																								// 与
																								// input
																								// 事件延时定时器
								$dropdownMenu = $input.parents('.input-group')
										.find('ul.dropdown-menu:eq(0)');
								...
								// 是否显示 button 按钮
								if (!options.showBtn) {
									$input.css('border-radius', '4px').parents(
											'.input-group:eq(0)').css('width',
											'100%').find('.btn:eq(0)').hide();
								}

								// 移除 disabled 类，并禁用自动完成
								$input.removeClass('disabled').prop('disabled',
				...
				disable : function() {
					return this.each(function() {
						$(this).attr('disabled', true).parent().find(
								'.btn:eq(0)').prop('disabled', true);
					});
				},
				enable : function() {
					return this.each(function() {
						$(this).attr('disabled', false).parent().find(
								'.btn:eq(0)').prop('disabled', false);
					});
				},
				destroy : function() {
					return this.each(function() {
						$(this).off().removeData('bsSuggest').removeAttr(
								'style').parent().find('.btn:eq(0)').off()
								.show().attr('data-toggle', 'dropdown').prop(
										'disabled', false) // .addClass('disabled');
			...
			$.fn.bsSuggest = function(options) {
				...
				if (typeof options === 'string' && methods[options]) {
					...
					return methods[options].apply(this, [].slice.call(
							arguments, 1));
				} else {
					// 调用初始化方法
