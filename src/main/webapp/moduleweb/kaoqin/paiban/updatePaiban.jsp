...
<body class="gray-bg">
						...
						<div class="row">
							...
							<div class="col-sm-5">
								...
								<div style="margin-top:25px;margin-left:20px">
									<button id="save" class="btn btn-primary" type="button"><strong>修改员工排班</strong></button>
								</div>
							</div>
						</div>
					</div>
	...
	<form id="form" >
	    <input id="_id" name="_id" type="hidden" value="${paibanId}" >
		...
		$(document).ready(
				function() {
					$('#calendar').fullCalendar(
					    ...
					    $("#paibanTitle").html("你正在修改 <span id='staffName1' style='color:blue'>"+'${staff.name}' +"</span>排班" + "<input type='hidden'  value="+'${staff._id}' +" >");
						var kPaiBan = $.parseJSON('${kPaiBanJSON}');
						$('#staffIds').val(kPaiBan.staffId);
						$('#staffNames').val(kPaiBan.staffName);
						$('#yearmonth').val(kPaiBan.yearmonth);
						for(var i =1; i<=31;i++) {
							if(i<10) {
								$("#day0"+ i).val(kPaiBan['day0'+i]);
							} else {
								$("#day"+ i).val(kPaiBan['day'+i]);
							}
						}
						var day = 1;
						$('.fc-widget-content').each(function(){
							if(!$(this).hasClass('fc-other-month')) {
								if(day >= 10) {
									if($("#day"+ day).val()!= "") {
										$(this).css('background-color', '#f4307b');
										$(this).css('color', '#000000');
										$(this).css('font-size', '14px');
										$(this).data("sel", true);
										$($(this).find(".fc-day-content div")).css(
												"text-align", "center").html( kPaiBan['day'+day +'B'] +"<input type='hidden' value="+ $("#day"+ day).val()+" >");
									}
			              	 	} else {
			              	 		if($("#day0"+ day).val()!= "") {
										$(this).css('background-color', '#f4307b');
										...
										$(this).data("sel", true);
			              	 		    $($(this).find(".fc-day-content div")).css(
											"text-align", "center").html(kPaiBan['day0'+day +'B'] +"<input type='hidden' value="+ $("#day0"+ day).val()+" >");
			              	 		}
			              	 	}
								day++;
							}
				        ...
				        $("#staffTable")
									.grid(
											{
												url : _ctxPath
														+ "/kaoqin/queryOneStaff.do?staffId="+"${staff._id}",
												shrinkToFit : false,
												colNames : [ "员工姓名","性别", "岗位"],
												colModel : [{
															name : "name",
				        	 ...
				        	 $("#save").click(function() {
								 ...
								 $.post(_ctxPath + "/kaoqin/upsertPaiban.do",$("#form").formobj(),function(data) {
								 	if(data ==  true) {
								 		layer.alert("修改排班成功！",function(index){
  											 layer.close(index);
  											 window.location.href = _ctxPath+ "/kaoqin/toQueryPaiban.do";
										});
								 	} else {
								 		 layer.alert("修改排班失败！",function(index){
  											 layer.close(index);
									     });
								  	}
								 }); 
