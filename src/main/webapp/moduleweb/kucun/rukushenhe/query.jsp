				...
				<div class="ibox float-e-margins">
					<div class="ibox-content">
						...
						<div class="jqGrid_wrapper">
							<table id="rukushenheTable"></table>
						</div>
					</div>
				</div>
			</div>
						...
						function() {
							...
							$("#rukushenheTable")
									.grid(
											{
												url : _ctxPath
														+ "/kucun/rukushenhe/query.do",
												shrinkToFit : false,
												postData : $("#searchForm")
														.formobj(),
												colNames : [ "操作","单号",<c:if test='${HQ==true}'>"退货","退货公司名称",</c:if>"物品类别","产品编码","产品名称","进货价格","进货数量","总价","产品产地","联系人","产品公司地址","电话","传真","邮箱","进货人","备注","产品状态","每瓶净含量","总量","部门","助记符","进货时间","有效期","单位","进货单位名称","组盘号","规格","入库审核","销售价格","销售总价","品牌","分公司入库审核","日期" ],
												colModel : [
															{
																name : "_id",
																index : "_id",
																...
																formatter : function(
																		...
																		options,
																		rowObject) {
																		var v = "<a href=\"javascript:checkRuku('"
																		+ cellvalue
																		+ "','"
																		+ rowObject.return_flag
																		+ "')\">审核"
																		+ "</a>&nbsp;&nbsp;";
		...
		function removeRuku(rukuId) {
			...
			$.get(url, {}, function(data) {
				if (data) {
					...
					$("#rukushenheTable").trigger("reloadGrid");
				} else {
					toastr.error("操作失败");
				}
			});
		}
		function checkRuku(rukuId,returnFlag) {
			var url = _ctxPath + "/kucun/ruku/check/" + rukuId + "/"+returnFlag+".do";
			$.get(url, {}, function(data) {
				if (data) {
					if (data.success) {
						toastr.success("操作成功");
