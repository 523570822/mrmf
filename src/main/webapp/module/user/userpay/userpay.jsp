...
<body class="bg_gray">
	<form action="" method="post" id="userpay_form">
        ...
        <div class="amount_right">
        	...
        	<input type="hidden" value="${user.walletAmount}" id="walletAmount"/> 
        </div>
    </div>
    	...
    	$(function(){
			$("#turn_money_btn").click(function() {//发送询价
				...
				$("#turn_money_btn").unbind("click");
				  if("2"==payType){
					$.post(_ctxPath + "/w/pay/balancePayOrder",{'userId':userId ,'organId':organId,'price':price},
					  function(data){
						if(data.status == "1"){
							wx.ready(function(){
								wx.chooseWXPay({
								    ...
								    cancel: function (res) {
								    	window.location.href = _ctxPath + "/w/pay/cancelOrder?orderId=" + data.params.userPayOrder._id;  
	                  				}
