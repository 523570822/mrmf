	...
	<script type="text/javascript">
    	...
    	$(function(){
            ...
            if(${mycoupon.moneyType==1}){//判断优惠券是打折券还是代金券
                ...
                salePrice = accSub(salePrice,moneyOrRatio);
            }
			...
			if(salePrice<walletAmount){
			  ...
			  if(confirm(tit)){//判断用户对弹出窗口的选择(true为确定)
				...
				$.post(_ctxPath + "/w/pay/balancePayOrder",{'userId':userId ,'organId':organId,'price':price,'orderId':orderId,'couponId':couponId},
				  function(data){
					if(data.status == "1"){
						wx.ready(function(){
							wx.chooseWXPay({
							    ...
							    success: function(res) {
				                    window.location.href=_ctxPath + "/w/userMy/myOrder.do?orderId="+orderId+"&price="+ salePrice;
							    },
							    cancel: function (res) {
		...
		function accSub(arg1, arg2) {//支付金额和钱包余额的算法
 		...
 		$("#couponList").click(function () {
            ...
            if (!regex.test(price)) {
                ...
                var userId = $("#user_id").val();
                var orderId=$("#order_id").val();
                var url = _ctxPath + "/w/module/coupon/queryCoupon.do?orderId="+orderId+"&price="+ price+"&userId="+userId;
                $.post(url,function (data) {
                    if(1 == data.status ){
                        layer.msg("<div style='text-align:center;'>无可用</div>");
                    }else {
                ...
                layer.msg("<div style='text-align:center;'>金额不可为0</div>");
            }
        });
