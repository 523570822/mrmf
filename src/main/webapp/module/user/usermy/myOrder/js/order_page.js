...
function orderList(){
	...
	if(pages!=""){
		...
		$.post(_ctxPath + "/w/userMy/myOrderList",{'userId':userId,'page':page,'type':type},
				function(data){
			var obj=eval(data);
			var html='';
			...
			if (obj.data.length==0) {
				html+='<li style="background:#eee;border:0;"><div class="notMyOrder"></div><div class="notMyOrderTitle">暂无相关订单</div></li>';
				$("#order_list").html($("#order_list").html()+html);
			}else {
				for(var i=0;i<obj.data.length;i++){
					html+='<li onclick="orderDetail(this)"><input type="hidden" value="'+obj.data[i]._id+'"><input type="hidden" value="'+obj.data[i].type+'"><div class="name"><div class="col-1"><img src="';
					...
					+obj.data[i].organName+'</div></div><div class="bespeakTime"><span class="colorGray">预约时间</span><span>'+obj.data[i].orderTime+'</span></div><div class="bespeakTime"><span class="colorGray">店铺地址</span><span class="appAddress">'
...
function orderDetail(str) {
	 ...
	 var type=$(str).children('input:eq(1)').val();
