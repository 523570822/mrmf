$(function(){
	...
	$("#back").click(function() {
		...
		window.location.href = _ctxPath + "/w/user/myvipInCard.do?userId="+userId+"&cardId="+cardId;
	});
});
function cardDetailList(){
	var userId=$("#user_id").val();
	var incardId=$("#incard_id").val();
	var page=$("#page").val();
	var pages=$("#pages").val();
	var type=$("#type").val();
	var cardType="children";
	if(page!=""){
	...
	/*console.info("incardId : " + incardId );
	console.info("page : " + page );*/
	if (type==1) {//会员详情
		$.post(_ctxPath + "/w/user/inCardDetailList",{'incardId':incardId,'page':page},
				function(data){
			var obj=eval(data);
			var html='';
			$("#page").val(data.page);
		...
		$.post(_ctxPath + "/w/user/customList",{'cardId':incardId,'page':page,'cardType':cardType},
				function(data){
			var obj=eval(data);
			var html='';
		...
		var cardType="child";
		$.post(_ctxPath + "/w/user/rechargeList",{'userId':userId,'cardId':incardId,'page':page,'cardType':cardType},
				function(data){
			var obj=eval(data);
			var html='';
			$("#pages").val(data.pages);
...
function cardStore() {
	var userId=$("#user_id").val();
	var cardId=$("#incard_id").val();
	var card="children";
	window.location.href = _ctxPath + "/w/user/cardStore.do?userId="+userId+"&cardId="+cardId;
}