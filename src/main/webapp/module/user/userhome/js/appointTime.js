$(function() {
	...
	$('#week').find('li').click(function() {
		...
		var caseId = $(this).find('input').eq(4).val();
		$('#dayF').val($.trim(day));
		$('#staffIdF').val($.trim(staffId));
		$('#organIdF').val($.trim(organId));
		$('#organNameF').val($.trim(organName));
			...
			var html = '';
			 if(obj.time8 == true) {
			    ...
			    $("#time li").each(function(index){
					...
					var start=$("#start").val(); 
					var end=$("#end").val(); 
					if(Number(intTime)>=index+9 && intDay==dayF){
					...
					if (start !="" && end !="") {
						if (Number(start)<=index+9 && Number(end)>=index+9) {
							$(this).addClass("busy");
						}
					}
				});
