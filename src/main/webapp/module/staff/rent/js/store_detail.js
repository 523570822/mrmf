$(function(){
	...
	$("#organ_detail_back").click(function(){
		...
		// } else {
			...
			if(search!=""){
				$("#organ_detail_form").attr("action",_ctxPath + "/w/staff/toSearchOrgan").submit();
			}else{
				$("#organ_detail_form").attr("action",_ctxPath + "/w/staff/toOrganList.do").submit();
			// }
	...
	$(".hair_collect").click(function(){
		var state=$.trim($(".hair_collect span").text());
		...
		if("收藏"==state){
			$.post(_ctxPath + "/w/organ/favorTheOrganId",{"organId":organId,"state":"1"},
					  ...
					  $(".hair_collect i").text('('+(followCount+1)+')');
