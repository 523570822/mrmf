$(function() {
	$('#com_dona').click(function() {
		...
		$.post(_ctxPath+"/w/organ/commitDonation",{'phone':phone}, function(data) {
			if(data.status  == 1) {
				window.location.href=_ctxPath+'/w/organ/comfirtDonation?phone='+phone;
				//跳转到输入转赠金额界面
				...
				window.location.href=_ctxPath+'/w/organ/toSetPayPassword?phone='+phone;
			} else {
				alert('对不起,程序内部有异常了');
			}
